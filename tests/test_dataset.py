...
class TestDataset:
    ...
    def test_nourish_dir(self, tmp_path, gmb_schema):
        ...
        assert str(e.value) == f'"{nourish_dir}" exists and is not a directory.'
    ...
    def test_dataset_download(self, tmp_path, gmb_schema):
        ...
        assert len(list(data_dir.iterdir())) == 2  # 'groningen_meaning_bank_modified' and '.nourish.dataset'
    ...
    def test_cache_dir_is_not_a_dir(self, tmp_path, gmb_schema):
        ...
        (tmp_path / '.nourish.dataset').touch()  # Occupy this path with a regular file
        with pytest.raises(NotADirectoryError) as e:
            Dataset(gmb_schema, data_dir=tmp_path, mode=Dataset.InitializationMode.DOWNLOAD_ONLY)
        assert str(e.value) == f"\"{tmp_path/'.nourish.dataset'}\" exists and is not a directory."
    ...
    def test_directory_write_lock_present(self, downloaded_gmb_dataset):
        ...
        lock_file = downloaded_gmb_dataset._nourish_dir_ / 'write.ding-dong.lock'
        lock_file.touch()
        ...
        self._test_lock_exception(
            ...
            write=True,
            directory=downloaded_gmb_dataset._nourish_dir_)
        assert f_stat == downloaded_gmb_dataset._file_list_file_.stat()  # _file_list_file_ wasn't overwritten

        self._test_lock_exception(
            lambda: downloaded_gmb_dataset.load(), write=False, directory=downloaded_gmb_dataset._nourish_dir_)
        with pytest.raises(RuntimeError) as e:
        ...
        self._test_lock_exception(
            lambda: downloaded_gmb_dataset.delete(), write=True, directory=downloaded_gmb_dataset._nourish_dir_)
    ...
    def test_directory_read_lock_present(self, downloaded_gmb_dataset):
        ...
        lock_file = downloaded_gmb_dataset._nourish_dir_ / 'read.king-kong.lock'
        lock_file.touch()
